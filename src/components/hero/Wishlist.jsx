...
const Wishlist = () => {
  ...
  return (
    <div className="w-full min-h-[100vh] bg-brand flex flex-col justify-start items-center text-white pt-8 pb-8 md:pb-24">
      <button
        onClick={() => navigate(-1)}
        className="absolute top-32 right-4 md:right-20 bg-transparent transition-all duration-.3s  hover:text-bck w-[35px] md:w-[50px] h-[35px] md:h-[50px] radius-50 rounded-[50px] flex items-center justify-center mb-2 border border-white"
      >
        <TfiClose className="fill-white" />
      </button>
      <div className="w-11/12 md:w-5/6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
        {wishData.map((el) => {
          return (
